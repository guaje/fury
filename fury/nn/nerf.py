...
class NeRFModel(nn.Module):
    def __init__(
        self, embedding_dim_pos=10, embedding_dim_direction=4, hidden_dim=128
    ):
        ...
        self.block3 = nn.Sequential(
            nn.Linear(
                embedding_dim_direction * 6 + hidden_dim + 3, hidden_dim // 2
            ),
            nn.ReLU(),
        )
...
def compute_accumulated_transmittance(alphas):
    ...
    return torch.cat(
        (
            torch.ones(
                (accumulated_transmittance.shape[0], 1), device=alphas.device
            ),
...
def render_rays(
    ...
    resp = c
    resp += 1 - weight_sum.unsqueeze(-1)  # Remove if not white background
...
def train(
    ...
    for _ in tqdm(range(nb_epochs)):
        for batch in data_loader:
            ...
            loss = (
                (ground_truth_px_values - regenerated_px_values) ** 2
            ).sum()
...
@torch.no_grad()
def test(
    hn, hf, dataset, chunk_size=10, img_index=0, nb_bins=192, H=400, W=400
):
    ...
    plt.imshow(img)
    plt.savefig(
        os.path.join("novel_views", f"img_{img_index}.png"),
        bbox_inches="tight",
    )
    plt.close()
...
if __name__ == "__main__":
    ...
    training_dataset = torch.from_numpy(
        np.load("training_data.pkl", allow_pickle=True)
    )
    testing_dataset = torch.from_numpy(
        np.load("testing_data.pkl", allow_pickle=True)
    )
