...
def test_new_sphere():
    ...
    print(np.sqrt(np.sum((centers - scene.GetActiveCamera().GetPosition()) ** 2, axis=1)))

    dots_actors = actor.new_sphere(centers, colors, scene)
    scene.add(actor.axes())
    scene.add(actor.sphere(np.zeros((1, 3)), np.zeros((1, 3)), radii=.1))
